...
def foodHeuristic(state, problem):
    ...
    food_uneat = foodGrid.asList()
    totCost = 0.0
    curr_point = position
    while food_uneat:
        heuristic_cost, food = min([(util.manhattanDistance(curr_point, food),food )for food in food_uneat])
        food_uneat.remove(food)
        curr_point = food
        totCost += heuristic_cost
    ...
    closestFood = util.nearestPoint(position, food_uneat)
    farthestFood = util.farthestPoint(position, food_uneat) 
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        for dot in foods:
            if dot not in un_eaten:
                un_eaten.append(dot)
        for food in un_eaten:
            distance = mazeDistance(startPosition,food,problem)"""
